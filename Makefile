#MODEL_FORMULATION = 
MODEL_FORMULATION = -DCAM_DYCORE -DUSE_PIO2 -DMPAS_NO_ESMF_INIT
...
ifeq "$(USE_PIO2)" "true"
	...
	CPPINCLUDES = -I$(PIO)
	FCINCLUDES = -I$(PIO)
	ifneq ($(wildcard $(PIO)/libpioc.a), ) # CHECK FOR SEPARATE C AND FORTRAN LIBS
		LIBS = -L$(PIO) -lpiof -lpioc
	else
		LIBS = -L$(PIO) -lpio
	endif
	ifneq ($(wildcard $(PIO)/libgptl.a), ) # CHECK FOR TIMING LIBRARY
		LIBS += -lgptl
	endif
endif
endif

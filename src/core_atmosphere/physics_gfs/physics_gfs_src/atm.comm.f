...
!fy   call uninterpred_cc(1,ISLM_RG,SLM1,SLM2)
...
!fy   call interpred_cc(1,ISLM_FG,SLM2,SLM0)
...
!fy   call interpred_cc(iiord,kmsk,f1,f)
...
!fy   call uninterpred_cc(iord,kmsk,f1,f2)
                ! <- interpolation FROM reduced grid (i.e. with # of
                ! longitudes varying from lat. circle to lat. circle)
                ! to full grid
!
!      print *,'in SEND_FLUX, before assemble_cc'
!fy   call  ASSEMBLE_cc(x,f2)
